...
const InvoiceWorkbench = () => {
    ...
    const handlePreview = (type: 'Prefactura' | 'Factura') => {
        ...
        const thresholdNum = quantityThreshold ? parseInt(quantityThreshold, 10) : 0;
        const percentNum = priceIncreasePercent ? parseFloat(priceIncreasePercent) : 0;
        ...
        entriesToInvoice.forEach(entry => {
            ...
            entry.items.forEach(item => {
                ...
                const deliveredQty = deliveriesForItem.reduce((sum: number, dItem) => sum + Object.values(dItem.sizeQuantities).reduce((qSum: number, q: unknown) => qSum + (Number(q) || 0), 0), 0);
